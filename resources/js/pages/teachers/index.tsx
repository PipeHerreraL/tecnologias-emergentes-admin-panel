  ...
  useEffect(() => {
    ...
    fetch(`/api/teachers?${query}`, { credentials: 'same-origin' })
      .then(async (r) => {
        ...
        if (ignore) return;
        const payload: Paginated<Teacher> = isWrapped<Paginated<Teacher>>(json)
          ? json.data
          : (json as Paginated<Teacher>);
            ...
            <table className="min-w-full text-left text-sm">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td className="px-4 py-6" colSpan={4}>No teachers found.</td></tr>
                ) : (
                  items.map((t) => (
                    <tr key={t.id} className="border-t border-sidebar-border/50">
                      ...
                      <td className="px-4 py-3">{t.subjects_count ?? 0}</td>
                    </tr>
                  ))
                )}
              </tbody>
